...
# INSERT INTO reference_data."USER_INPUT" ("year", quarter) VALUES(2021, 2);
...
print(f"We are cleaning STB_QUARTER_{QUARTER}_{YEAR} Survey Data")
...
def get_tables_with_quarter_year(cursor, schema_name, quarter, year):
    ...
    for table_name in table_names:
        ...
        if 'QUARTER' in column_names and 'YEARS' in column_names:
            ...
            cursor.execute(f'''SELECT 1 FROM {schema_name}."{table_name}" WHERE "QUARTER" = {QUARTER} AND "YEARS" = {YEAR} LIMIT 1''')
...
result_tables = get_tables_with_quarter_year(cursor, schema_name, QUARTER, YEAR)
...
# Print the result
print(f"Tables with {QUARTER} and {YEAR}:")
print(result_tables)
...
print(f'Multi final data for quarter {QUARTER} for year {YEAR} have been merged into 1 data frame ')
...
schema='production_micro_fc_stb_quarterly'
...
df_wpop.to_sql('JR4Q'+str(quarter_t01)+'Y'+years_t01+'_FC',con=engine,schema=schema,index=False,if_exists='replace')
